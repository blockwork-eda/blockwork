...
class Singleton(type):
    ...
    def __call__(cls, *args, **kwds) -> Any:
        if cls not in Singleton.INSTANCES:
            Singleton.INSTANCES[cls] = super().__call__(*args, **kwds)
        return Singleton.INSTANCES[cls]


class ParameterisedSingleton(type):
    INSTANCES: ClassVar[dict[tuple[type, str | int], Any]] = {}

    def __call__(cls, *args, **kwds) -> Any:
        uniq_key = (cls, *args, *(f"{k}={v}" for k, v in kwds.items()))
        if uniq_key not in ParameterisedSingleton.INSTANCES:
            ParameterisedSingleton.INSTANCES[uniq_key] = super().__call__(*args, **kwds)
        return ParameterisedSingleton.INSTANCES[uniq_key]


def keyed_singleton(
    arg_key: Callable[..., Hashable] | None = None,
    inst_key: Callable[[Any], Hashable] | None = None,
):
