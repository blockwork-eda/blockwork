...
class Bootstrap(RegisteredMethod):
    ...
    def __call__(self,
                 context : Context,
                 mode    : BwBootstrapMode = BwBootstrapMode.default) -> None:
        """
        ...
        if self.checkpoints:
            ...
            for chk in self.checkpoints:
                ...
                if (chk_path.exists() and
                    datetime.fromtimestamp(chk_path.stat().st_mtime) <= last_run):
                    logging.debug(f"Bootstrap step '{self.full_path}' checkpoint "
